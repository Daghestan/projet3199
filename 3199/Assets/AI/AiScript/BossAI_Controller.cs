...
public class BossAI_Controller : MonoBehaviour, HumanoidInterface
{
    ...
    private bool Monologing = true; // Vérifie si le bot est toujours en train de parler. Tant qu'il parle, il est immortel mais ne peut rien faire.
    public List<string> Chat;
    
    private Transform canvaHolder;
    ...
    IEnumerator ShowDialog()
    {
        foreach (string sentence in Chat)
        {
            ...
            {
                dialog.text += letter;
                yield return new WaitForSeconds(0.05f); // Wait before showing next letter
            }
            yield return new WaitForSeconds(0.5f); // Wait before showing next sentence
        }
        Monologing = false; // Set Monologing to false after showing all texts
    }
